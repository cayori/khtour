...
<link rel="stylesheet" type="text/css" href="/khtour/css/input.css" />
...
<div id="container">
	...
	<div class="user_info_edit">
		<p class="info_edit_title"><span>회원탈퇴</span></p>
		<div class="login_box">
			<form method="post" action="DeleteMember.action" name="form" id="login_form" autocomplete="off" onsubmit="">	
				<div>
					<input type="hidden" name="action" id="action" value="certify">
					<label class="title">아이디</label>
					<span class="input_id"><s:property value="#session.id"/><input type="hidden" name="id" id="login_id" value="<s:property value="#session.id"/>"></span>
					<label class="title">비밀번호</label>
					<span class="input_id"><input type="password" name="password" id="login_pw" title="비밀번호" style="height: 25px;"/></span>
				</div>
				<p class="coment">
					외부로부터  <span><s:property value="#session.id"/></span> 님의 정보를 안전하게 보호하기 위해 비밀번호를 다시 한번 확인합니다.<br />
