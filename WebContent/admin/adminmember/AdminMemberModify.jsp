...
function TTNBClick () {
	...
	function notValidContentsDel(id){
		$('div.tagit-label','#'+id).each(function(){
			var thisItem = $(this);
			var content = thisItem.text();
			if(in_array(content,cityNames)){
  				var index = cityNames.indexOf(content);
  				var value = cityCodes[index];
				thisItem.parent().attr('tagvalue',value);
			}else{
	...
	$(document).ready(function(){
	    ...
	    /*
	  	$('#visitingCityV').tagit({tagSource:cityNames,tagsChanged:function(tagvalue,act){
	  		if(act == 'added'){
				notValidContentsDel('visitingCityV');
	  		}
	  	}});

	  	$('#visitingCityG').tagit({tagSource:cityNames,tagsChanged:function(tagvalue,act){
	  		if(act == 'added'){
				notValidContentsDel('visitingCityG');
	  		}
	  	}});
        */

	  	$('#imageChangeBtn').singleUploaded("https://www.tourtips.com/ap/members/imageup/?act=imageup", function(data){
			var data = eval("["+data+"]");
			imgData = data[0];
			if(!imgData.result){
				alert(imgData.message);
			}else{
//				$('#userProfileImage').attr('src',imgData.url).attr('fpath',imgData.path);
				$('#userProfileImage img').attr("src", imgData.url+'&t=b');//파라미터 t=b 추가 --> 큰사이즈 섬네일
				$('#common_user_pic').attr('src','').attr('src',imgData.url);
			}
			//$('#userProfileImage img').imageCenterFixed(75, 75);
		});

		$('#imageDeleteBtn').click(function(){
			if(confirm('프로필 사진을 삭제 하시겠습니까?')){
				$.ajax({
					url:'https://www.tourtips.com/ap/members/imageup/',
					data:{act:'imagedel'},
					dataType:'json',
					type:'post',
					success:function(data){
						if(data.rcode == '100'){
							...
							location.href='https://www.tourtips.com/ap/members/myinfo/';
						}
					}
				});
			}
			...
			<div id="content">
				<div class="body">
					...
					<div class="sbody">
						...
						<form name="form" method="post" action="AdminModifyMember.action" onsubmit="return checkmodify(this.form)">
							<div class="add_info">
								회원가입 시 입력했던 회원정보를 수정할 수 있습니다.<span> <!--<a href="javascript:closeAddInfo();"><img src="//img.tourtips.com/images/mypage/btn_close.gif" alt="닫기" /></a>-->
								</span>
							</div>
							<div class="join_form" style="margin-top: 0">
								<input type="hidden" name="token" id="token"
									value="IUv03CigF-VZz18E6fLOb6b4JcaMhZgWaoC3z0p-Gp75/1-a1eKaOXpRJfpotWH3bXymKcWie0X29wja">
								...
								<fieldset>
										...
										<table border="1" cellspacing="0" summary="회원가입 폼 작성">
											...
											<tbody>
													...
													<td>
														<div class="item">
															<input type="password" name="newpassword"
																maxlength="20" placeholder="비밀번호 변경 시 입력" style="height: 25px;"/>
														</div>
													</td>
												</tr>
												<tr>
													...
													<td>
														<div class="item">
															<input type="password" name="newpassword2" title="새 비밀번호"
																class="i_text" maxlength="20" placeholder="비밀번호 변경 확인" style="height: 25px;"/> <span class="i_tip"
																id="newPassConfirm_help" style="display: none"><img
													...
													<td>
														<div class="item">
															<input type="text" name="email" id="subemail_id"
																class="i_text" style="width: 100px" value="${email}" />

															@ <input type="text" name="email2" id="subemail_host"
																class="i_text" style="width: 100px" value="${email2}" />
												...
												<tr>
													...
													<td>
														<div class="item">
															<input type="text" name="tel" title="연락처" class="i_text"
																maxlength="11" value="${tel}" />
															<!-- 숫자만 들어가게 구현해보자 -->
														</div>
													</td>
												</tr>
									...
									<p class="join_btn_group">
										<span class="join_btn"><button type="submit"
												id="dataModifyBtn">수정</button></span>
									</p>
								</fieldset>
							</div>
						</form>
